/*
 * Esta clase concreta es la representacion de las Clases en la aplicacion
 * la cual es la ultima descendiente de la dinastia Componente-Vertice, sus metodos
 * son: Clase, dibujar, showDialogo
 * 
 */
 ...
 * 
 */
public class Clase extends AVertice {

	private boolean abstracto = false;
	private final Image imagen = Toolkit.getDefaultToolkit().getImage("P-Clase.jpg");
	private final Image imagenAbstracta = Toolkit.getDefaultToolkit().getImage("P-ClaseAbstracta.jpg");
	 ...
	 * @param al
	 */
	public Clase(int x, int y, int an, int al) {
		super(x, y, an, al);
	}
	...
	public void dibujar(Graphics Componente) {
		...
		if(abstracto){
			Componente.drawImage(imagenAbstracta,5,5,this);
		}
		else{
	...
	public void showDialogo() {
		...
		menu.add(nombre);
		menu.add(new JLabel("Abstracta:"));
		menu.add(abs);
		
		menu.add(aceptar);
		menu.add(cancelar);
		
		menu.setTitle("Clase - Propiedades");
